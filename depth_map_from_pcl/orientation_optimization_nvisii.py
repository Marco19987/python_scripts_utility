...
def orientation_cost_function(orientation):
    ...
    cost = cost/(h_image*w_image) + 1*abs(aspect_ratio_1-aspect_ratio_2)
...
# Pose real object
translation_real = np.array([0.2,-0.2,0.7]) # position of the object in meters wrt camera
...
def optimize(guess):
    # return minimize(orientation_cost_function, guess, method='Powell', tol=1, 
    #                 options={'ftol': 1,'xtol' : 1e-1, 'eps': 1e-1,'maxiter': 5,'disp': True}, constraints=module_constraint)
    theta = np.linalg.norm(guess)
