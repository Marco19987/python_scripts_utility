...
viewpoint_file = 'connectorMO6_viewpoints_45.pkl'
...
# Pose real object
translation_real = np.array([0,0,0.3]) # position of the object in meters wrt camera
euler_angles_real = [0.2,0.1,0.6] # radians - roll pitch and yaw

# import random
# euler_angles = [random.uniform(0, 2*np.pi),random.uniform(0, 2*np.pi),random.uniform(0, 2*np.pi)]
quaternion_real = euler_to_quaternion(euler_angles_real)#[0,0.5,0.5,0]  
...
costs = np.array([compute_cost(resize_images_to_same_size(obj_depth_image_normalized, element['depth_map'])[0],
                               resize_images_to_same_size(obj_depth_image_normalized, element['depth_map'])[1],obj_depth_image_normalized,element['depth_map'])
...
orientation_angles = np.array([element['euler_angles'] for element in selected_data])
...
ori_diff = []
for element in selected_data:
    ori_diff.append(np.linalg.norm(np.array(element['euler_angles']) - np.array(euler_angles_real)))
